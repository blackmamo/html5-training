...
describe("Matcher", function() {
    ...
    var idGenerator = jasmine.createSpyObj(['getTradeId','getOrderId'])
    idGenerator.getTradeId.and.callFake(function(){return idGenerator.nextTradeId})
    idGenerator.getOrderId.and.callFake(function(trader){return trader + idGenerator.nextOrderId})
    ...
    describe("when considering depth protocol", function() {
        ...
        it("removes depth when consumed, with no phantom updates", function() {
        ...
        it("provides depth snapshots", function() {
            ...
            expect(matcher.depthSnapshot()).toEqual(new DepthSnapshot(
                // bids
        ...
        it("provides depth snapshots ordered best to worst price", function() {
            addStartingDepth()
            ...
            expect(matcher.depthSnapshot()).toEqual(new DepthSnapshot(
                // bids
                [{price: 13.0, qty: 100}, {price: 12.0, qty: 3}, {price: 11.0, qty: 23}],
                // offers
                [{price: 15.0, qty: 19}, {price: 15.5, qty: 2}, {price: 16.0, qty: 100}]))
        })

        it("provides depth snapshots aggregated by price", function() {
            addStartingDepth()
        ...
        it("provides depth snapshots with filled orders removed", function() {
            addStartingDepth()
        ...
        it("provides depth snapshots that show partial fills", function() {
            addStartingDepth()
        ...
        it("Adds unfilled portions of an order to the book", function() {
    ...
    describe("when considering order protocol", function() {
        ...
        it("requests trade and order ids and uses them", function () {
        ...
        it("orders with exactly matching prices fill and updated as not live", function () {
            matcher.submit(new OrderRequest("Garry", Sides.Bid, 13.0, 43))
            matcher.submit(new OrderRequest("Andrew", Sides.Offer, 13.0, 43))
            ...
            expect(orderUpdates[0]).toEqual(partially(
                {trader: "Garry", side: Sides.Bid, price: 13.0, reqQty: 43, remainingQty: 43, live: true}
            ))
            expect(orderUpdates[1]).toEqual(partially(
                {trader: "Andrew", side: Sides.Offer, price: 13.0, reqQty: 43, remainingQty: 43, live: true}
            ))
            expect(orderUpdates[2]).toEqual(
                new Fill(0, "Andrew", "Andrew0", "Garry", "Garry0", 13.0, 43))
            expect(orderUpdates[3]).toEqual(partially(
                {trader: "Garry", side: Sides.Bid, price: 13.0, reqQty: 43, remainingQty: 0, live: false}
            ))
            expect(orderUpdates[4]).toEqual(partially(
                {trader: "Andrew", side: Sides.Offer, price: 13.0, reqQty: 43, remainingQty: 0, live: false}
            ))
        })

        it("orders with inexactly matching prices fill at the price of the older order", function () {
        ...
        it("retains partially filled orders", function () {
            ...
            var expected = [
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 13.0, reqQty: 100, remainingQty: 100, live: true}),
                //first fill
                partially(
                    {trader: "Garry", side: Sides.Bid, price: 13.0, reqQty: 10, remainingQty: 10, live: true}),
                new Fill(0, "Andrew", "Andrew0", "Garry", "Garry0", 13.0, 43),
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 13.0, reqQty: 100, remainingQty: 90, live: true}),
                partially(
                    {trader: "Garry", side: Sides.Bid, price: 13.0, reqQty: 10, remainingQty: 0, live: false}),
                // second fill
                partially(
                    {trader: "Garry", side: Sides.Bid, price: 13.0, reqQty: 10, remainingQty: 10, live: true}),
                new Fill(0, "Andrew", "Andrew0", "Garry", "Garry0", 13.0, 43),
            ...
            for (var i = 0; i++; i < expected.length){
                expect(orderUpdates[i]).toEqual(expected)
            }
        })

        it("matches orders against the whole opposing side, status after fills", function () {
            ...
            var expected = [
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 13.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 15.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 14.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Guy", side: Sides.Bid, price: 20.0, reqQty: 30, remainingQty: 30, live: true}),
                // The fills
                new Fill(0, "Guy", "Guy0", "Andrew", "Andrew0", 13.0, 10),
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 13.0, reqQty: 10, remainingQty: 0, live: false}),
                new Fill(0, "Guy", "Guy0", "Andrew", "Andrew0", 14.0, 10),
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 14.0, reqQty: 10, remainingQty: 0, live: false}),
                new Fill(0, "Guy", "Guy0", "Andrew", "Andrew0", 15.0, 10),
                partially(
                    {trader: "Andrew", side: Sides.Offer, price: 15.0, reqQty: 10, remainingQty: 0, live: false}),
                partially(
                    {trader: "Guy", side: Sides.Bid, price: 20.0, reqQty: 30, remainingQty: 0, live: false})
            ]
            expect(orderUpdates.length).toEqual(expected.length)
            for (var i = 0; i++; i < expected.length){
                expect(orderUpdates[i]).toEqual(expected)
        ...
        it("matches against opposing orders in FIFO order, status after fills", function () {
            ...
            var expected = [
                partially(
                    {trader: "Andrew", side: Sides.Bid, price: 13.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Sonia", side: Sides.Bid, price: 14.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Sam", side: Sides.Bid, price: 13.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Barney", side: Sides.Bid, price: 14.0, reqQty: 10, remainingQty: 10, live: true}),
                partially(
                    {trader: "Guy", side: Sides.Bid, price: 10.0, reqQty: 50, remainingQty: 350, live: true}),
                ...
                partially(
                    {trader: "Sam", side: Sides.Offer, price: 14.0, reqQty: 10, remainingQty: 0, live: false}),
                new Fill(0, "Guy", "Guy0", "Sonia", "Sonia0", 14.0, 10),
                partially(
                    {trader: "Sonia", side: Sides.Offer, price: 14.0, reqQty: 10, remainingQty: 0, live: false}),
                new Fill(0, "Guy", "Guy0", "Barney", "Barney0", 14.0, 10),
                partially(
                    {trader: "Barney", side: Sides.Offer, price: 14.0, reqQty: 10, remainingQty: 0, live: false}),

                // the update for guy's order should show it is still live as it isnt filled
                partially(
                    {trader: "Guy", side: Sides.Bid, price: 10.0, reqQty: 50, remainingQty: 10, live: true})
            ]
            expect(orderUpdates.length).toEqual(expected.length)
            for (var i = 0; i++; i < expected.length){
                expect(orderUpdates[i]).toEqual(expected)
